...
def setup(rank, world_size):
    ...
    torch.cuda.set_device(rank + 1)  # Set to GPU 1, 2, 3, or 4
...
def train_model(rank, world_size, train_data_dir, val_data_dir, epochs=4, batch_size=32, learning_rate=0.0005, all_metrics=None):
    ...
    try:
        ...
        device = f"cuda:{rank + 1}" if torch.cuda.is_available() else "cpu"
        print(f"Currently running on {world_size} {'GPUs' if torch.cuda.is_available() else 'CPU'}")
        print(f"Running GPU {rank + 1 if torch.cuda.is_available() else 'CPU'} (rank {rank + 1})")
        ...
        model = DDP(model, device_ids=[rank + 1], find_unused_parameters=False)
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        loss_function = nn.CrossEntropyLoss(weight=torch.tensor([0.5, 10, 100]).to(device))

        start_time = time.time()
        gpu_metrics = {'rank': rank + 1, 'epochs': []}
        ...
        with open(f'results/metrics_gpu{rank + 1}.json', 'w') as f:
...
def main():
    ...
    for gpu in gpus:
        mp.spawn(train_model, args=(gpu, train_data_dir, val_data_dir, epochs, batch_size, learning_rate, all_metrics), nprocs=1, join=True)

    # Save final metrics to a JSON file
        os.makedirs('results', exist_ok=True)
        with open(f'results/final_metrics_{gpu}.json', 'w') as f:
